...
`latest_version` is the bundle whenever it is newer than what the receiver runs and the signed
index has not withdrawn it.
...
`.dirty` when its tree was not clean) and a release build as `0.3.0`. A development build is never
current against the release of its own number, so the card offers the release back; two builds of
the same version are otherwise ordered by commit time. A plugin up to 0.3.x reports no build id
and is shown as the version it reports.
...
install above the one running - today only the bundle. A choice is stored in the entry's options
